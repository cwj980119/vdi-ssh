...
| 화면 접속 / SFTP·SCP / 포트 포워딩 / VS Code Remote SSH | 현재 미지원 |
...
서버 창에 Windows 실행 계정, 접속 주소, `SHA256:...` 호스트 키 지문이 표시됩니다. 이 창을 실행 상태로 유지합니다. `Ctrl+C`로 종료할 수 있습니다.
...
프로그램은 방화벽, Windows 서비스, 자동 시작 설정을 변경하지 않습니다. Windows 로그오프나 재부팅 후에는 다시 실행해야 합니다. VDI 클라이언트를 닫았을 때 세션이 유지되는지는 해당 VDI 정책에 따릅니다. 추가 진단과 제약은 [상세 설명서](README.ko.md)를 참고하세요.
...
개발 PC에서 공개키 인증·접속 거부, 한글 명령 출력, 대화형 터미널, 세션 정리, Windows OpenSSH 클라이언트 호환성을 포함한 통합 테스트 9개를 통과했습니다. 실제 VDI의 네트워크와 보안 정책에서의 동작은 해당 환경에서 확인해야 합니다. 배포 EXE는 코드 서명되지 않았습니다.
